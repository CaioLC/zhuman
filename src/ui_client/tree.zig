...
/// and drawn in order (later trees paint on top). A one-field wrapper (not a bare slice) so
/// the frame return type reads as a name (`Trees`) at the `main.zig` call site. Formerly
/// named `Ui`.
pub const Trees = struct { trees: []const *Node };
