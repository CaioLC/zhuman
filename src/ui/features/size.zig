...
pub const Size = struct {
    ...
    data_width: f32,
    data_height: f32,
    /// Cross-axis **alignment reference** for a row (`dir = .row`): the px offset of the
    /// reference line up from the *content's* bottom edge. `0` (the default) ⇒ the reference
    /// is the content bottom, so a plain box aligns by its bottom. The host sets it for text
    /// (= the font's descent) so a mixed-size row can share a common baseline — measured at
    /// build like `data_*` (the host knows nothing of padding then), so it's content-relative;
    /// the layout reads it via `baseline_off`, which folds the bottom padding back in.
    /// Consumed only by the placement pass under `Flow.cross = .baseline`; ignored by every
    /// other cross-align and by column flows.
    baseline: f32 = 0,
    ...
    /// The reference line's offset up from the **box's** bottom edge: the content-relative
    /// `baseline` plus the bottom padding (padding grows the box below the content, so the
    /// box-relative reference shifts with it). The baseline placement pass reads this, never
    /// `baseline` directly — a padded text chip still lines its glyphs onto the shared line.
    pub fn baseline_off(self: Size) f32 {
        return self.baseline + self.padding.down;
    }
