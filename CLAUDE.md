...
**Visual identity (M5):** the whole HUD is colored from the foundation's `Theme` roles (`src/ui_client/theme.zig`) filled in by the game's palettes (`src/palette.zig`) — `cold`/`warm` poles blended by `palette.lerp(compute_warmth(...))`, resolved once per frame in `build_ui` onto `Resources.theme` (default `theme.cold`) so every widget reads `ctx.res.theme.*` (fg/acc/dim/warn/danger/line/line2/panel/bg) instead of a fixed color — a rested actor reads warm, a spent one cold. `compute_warmth` (now in `src/pages/`) is currently just the vigor fraction (`vigor.v / vigor.max`) — the satiety/capital inputs it once blended went away with those mechanics, to be reworked when capital returns to the HUD. The font is monospace (`Kenney Mini Square Mono.ttf`); a `draw_scanlines` overlay (partial-alpha, needs the renderer's `.blend` mode set once in `App.init`) darkens every 4th pixel row on top of everything, last, each frame.
...
- `src/palette.zig` — the game's palettes: the `cold`/`warm` poles and `lerp(t)` blending them. Values only — the `Theme` *shape* they fill in belongs to the UI foundation (`src/ui_client/theme.zig`, which also owns `Color` and `mix`, and defaults every role to a plain neutral palette so the foundation renders with no game attached). A screen installs a palette by assigning it onto `res.view.theme`; `ui_client` never names a palette. See "Visual identity (M5)" above.
- `src/res.zig` — `Resources`, the host bundle held by `Ctx` as `*Res` and passed to systems. Grouped by **who writes it**: `platform` (font, renderer, window, textures — set at init, read only by the UI), `input` and `time` (the event loop, once per frame), `sim` (the run: `elapsed`, `log`, `tutorial_done`, `prng` — `sim.reset()` starts a fresh run, deliberately carrying the prng over so runs don't replay the same luck), `config` (tuning — `secs_per_day`, the condition thresholds `spent_frac`/`weary_frac` and labor's `weary_yield`, and the metabolism rates `starve_per_day`/`vigor_per_food`/`ration_scale`/`feast_scale`; outside `sim` so a start-over can't discard a player setting. `Config.condition(frac)` is the one definition of the ALIVE/WEARY/SPENT bands — the status word, the vigor chip's color, the hunger log lines and `actions.yield_factor` all read it, so they can't drift), and `view` (this frame's resolved `theme`, recomputed in `build_ui`; nothing here persists across frames). `res.random()` is the sim's single source of chance (uncertain action outcomes today; AI deciders later).
...
- `src/root.zig` — library (`ha`) root, re-exports `sdl`, `ui`, `ui_client`, `comp`, `tag`, `log`, `dist`, `palette`, `res`, `world`, `ecs`, `systems`, `actions`, `capital`. `build.zig` also makes the module import *itself* as `ha`, so library-internal files (`components`/`actions`/`capital`) can use `@import("ha")`.
