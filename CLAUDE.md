...
`test-ui` compiles the reusable UI layer (`src/ui/` + `src/ui_client/` minus `pages.zig`)
in isolation, so it stays green even while the sim half (`main.zig`/`systems.zig`) is
mid-refactor and the full `zig build test` is red.
