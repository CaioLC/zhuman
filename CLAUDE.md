...
7. **Render** — userland render loop in `main.zig`: `root.iterate()` walks the tree and draws each node by its `render_data` aspects, unwrapping each optional payload for its color (e.g. `if (node.render_data.text) |c| widgets.draw_text(..., c)`); then `ui.endFrame()`
