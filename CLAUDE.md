...
The game's design — the vision, the two pillars, the locked decisions and the Act structure — is [`docs/design.md`](docs/design.md); what's next is [`docs/roadmap.md`](docs/roadmap.md). The near-term items below are scoped but not yet built:

- **Balance the metabolism numbers** — the vigor-drain/death path and the ration control both landed 2026-08-15 as `systems.metabolize` (continuous eating at `comp.Metabolism.setting`'s rate; starvation drains 4 vigor/day on an empty larder, so `mark_dead` finally fires). Two-level labor quality landed the same day: `actions.yield_factor` scales yields by ×0.7 below the WEARY threshold (35%) — a constant step, deliberately *not* the linear slide (built once, reverted: band churn + felt bad); both `gather`'s draw and the tiles' displayed bands call the one helper so promise and draw can't drift, and quality is judged *before* the click's cost is paid. Action durations landed 2026-08-16: `Requires.hours` (Forage 4h / Fish 5h / Chop 6h / rod build 12h; a day = 24h over `secs_per_day`), the begin/finish split with `comp.Busy`, the time token in the tile grammar (`-2 4h ⌒ +1-3f`), the underbar progress fill on the running tile, and the full-chip eating pulse on the ration dial's active option (fill = `ceil(F) − F`, progress through the current food unit — spoilage shows in it, an empty larder stops pulsing). The rates (`base_rate` 1.5 food/day, ½×/1×/2× multipliers, starve 4/day, spoilage 0.05/s, weak ×0.7, the hour costs) are first guesses — playtesting will retune. Cancellation of a running act is deliberately not built (refund questions can wait until someone feels trapped by a 12h build).
